...
class BookUI:
    ...
    def input_book_data(self, existing_book: dict = None) -> dict:
        ...
        title = input(f"Название: ({existing_book['title']}) " if existing_book else "Название: ")
        author = input(f"Автор: ({existing_book['author']}) " if existing_book else "Автор: ")

        year_str = input(f"Год: ({existing_book['year']}) " if existing_book else "Год: ")
        genre = input(f"Жанр: ({existing_book.get('genre')}) " if existing_book and existing_book.get('genre') else "Жанр: ")
